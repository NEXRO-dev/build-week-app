...
function timeline(plan: TomorrowPlan): TimelineItem[] {
  return [
    ...plan.keep.map((item) => ({ id: item.id, time: item.proposedTime ?? item.originalTime ?? "未定", title: item.title, kind: "keep" as const, detail: item.reason })),
    ...plan.move.map((item) => ({ id: item.id, time: item.proposedTime ?? item.originalTime ?? "未定", title: item.title, kind: "move" as const, detail: item.reason })),
    ...plan.restBlocks.map((item) => ({ id: item.id, time: item.startTime, end: item.endTime, title: "休息ブロック", kind: "rest" as const, detail: item.reason })),
    ...plan.reschedule.map((item) => ({ id: item.id, time: item.originalTime ?? "未定", title: item.title, kind: "reschedule" as const, detail: `${item.proposedTime ?? "翌日以降"}へ延期` })),
  ...
  return (
    <div>
      ...
      <div className="px-4 pb-8 pt-4">
        <p className="mb-4 text-center text-xs font-semibold text-[#545d7d]">無理なく進めるための予定です</p>
        ...
        <section className="mt-5 rounded-lg border border-[#e3e5ef] p-4">
          ...
          <ul className="mt-3 space-y-2">
            ...
            {!plan.keep.length ? <li className="text-xs text-[#717997]">維持する予定はありません</li> : null}
          </ul>
        </section>

        {plan.reschedule.length ? (
          <section className="mt-3 rounded-lg border border-[#e3e5ef] p-4">
            <h2 className="text-xs font-bold">延期候補</h2>
        ...
        <Button variant="primary" size="lg" fullWidth onPress={onApproval} className="mt-5 h-12 bg-[#5b42ff] text-white">変更内容を確認<ArrowRight size={18} /></Button>
      </div>
    </div>
  );
}
